...
public class GraphArt
{
    ...
    private String rootDir = "graph_art_drawing_samples";
    private String executable;
    
    private boolean graphVizExists = true;

    public GraphArt() {
        this(null, null);
    }
    
    public GraphArt(String executable) {
    	this(executable, null);
    }
    
    public GraphArt(String executable, String rootDir) {
	 ...
	 * @param Object graph
	 */
    public void drawGraph(String fileName, Object graph) {
    ...
    public void drawGraph(Object graph) {
	 ...
	 */
    public void drawGraph(String fileName, String dotString) {
    	int dotId;
        try {
        	dotId = writeDotToFile(fileName, dotString);
    ...
    public void drawGraph(String dotString) {
    	drawGraph(null, dotString);
    }
    ...
    {
    	if(!graphVizExists) {
    		System.err.println("GraphViz dot not found on expected location C:/Program Files/GraphViz... \n"
				+ "or C:/Program Files (x86)/GraphViz... on Windows. On MacOSX and Linux it should be on path.\n"
				+ "If you installed GraphViz somewhere else use constructor GraphViz(String executablePath).\n"
				+ "If you do not have GraphViz installed on your computer please\n"
				+ "install GraphViz from https://graphviz.org/.\n\n"
				+ "######### IMPORTANT #########\n"
				+ "If for some reason you don't want to install GraphViz, dont use drawGraph(...),\n"
    ...
    {
    	//dotId is irrelevant when filename is present, so it is set to be 0
    	int dotId = 0;
    	if(filename == null) {
    		dotId = updateDotId();
    	} 
     ...
     * @param string (dot)
     */
    public void drawInBrowser(String dot) {
    	try {
    		String dotString = URLEncoder.encode(dot.replace("\s", "")
    				.replace("\n", "").replace("\t", ""), "UTF-8");
        	String url = GRAPH_DRAWING_ONLINE_SERVICE+dotString;
			Desktop.getDesktop().browse(new URL(url).toURI());
		} catch (IOException | URISyntaxException e) {
			e.printStackTrace();
		}
    ...
    /**
     * For edge weighted graph and digraph creates string of vertices 
     * @param graph
     * @param className
     * @return
     * @throws NoSuchMethodException
    ...
    /**
     * Creates glue between vertices in graph/digraph
     * @param className
    ...
    /**
     * Checks if class is graph or digraph and sets beginning of dot string
     * @param className
     * @return
     * @throws ClassNotFoundException
     */
